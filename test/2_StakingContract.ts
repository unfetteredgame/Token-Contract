...
describe('Staking Contract', () => {
	...
	describe("\n\n#########################################\n stake function\n#########################################", () => {
		it("rejects staking if token amount less then minimum staking amount", async () => {
		...
		it("rejects staking with invalid duration parameter (should accept only 1, 3, 6 and 12)", async () => {
	...
	describe("\n\n#########################################\n withdraw function\n#########################################", () => {
		it("rejects withdrawing before release date", async () => {
			await proxy.connect(owner).transferSoulsToAddress(staker.address, ethers.utils.parseEther("50000"));
			await souls.connect(staker).approve(staking.address, ethers.constants.MaxUint256);
		...
		it("rejects withdrawing if the stake has already withdrawn", async () => {
			await proxy.connect(owner).transferSoulsToAddress(staker.address, ethers.utils.parseEther("50000"));
			await souls.connect(staker).approve(staking.address, ethers.constants.MaxUint256);
	...
	describe("\n\n#########################################\n emergencyWithdraw function\n#########################################", () => {
		it("rejects emergency withdrawing after release date", async () => {
			await proxy.connect(owner).transferSoulsToAddress(staker.address, ethers.utils.parseEther("50000"));
			await souls.connect(staker).approve(staking.address, ethers.constants.MaxUint256);
